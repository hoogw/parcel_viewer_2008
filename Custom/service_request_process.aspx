...
session("Fld_DBNAMES")="RequestID, RequestDate, ST_NUMBER, LONG_ADDRESS, UNIT_DESIGNATOR, RequestLocation, CompletedDate, ReferralDate,"
...
session("DBTbl")="vwServiceRequest"
''''''''''''''''''''''''''''''''''''''''''
...
function SRDetails(n) {
	document.location="custom_service_request_details.aspx?pid=" + n;
}
...
'	if CInt(Request.Form("FieldCount"))=0 then
'		response.write ("No Service Requests found!")
'		response.end
'	end if
...
'        response.write ("&nbsp;<center><input type=button class=Btn onclick=""document.location='" & StartOverASP & "'"" Value=""Start Over""></center>&nbsp;")
	...
	response.write ("<TH class=tblHdr>Service Request Detail</TH>")
...
'			response.write ("<TD><center><a href='javascript:SendValue(""" & Trim(CStr(session("Fld_SHP"))) & """,""" & IDValue & """,""" & CustLayer & """);'>Rec " & n & "</a></center></TD>")
			...
			response.write ("<TD width=85><nobr><center><a href=""javascript:SRDetails('" & IDValue & "');"">" & DT.Rows(i)(0) & "</a></center></nobr></TD>")
			response.write ("<TD width=85><nobr><center><a href=""javascript:SIRE('" & IDValue & "',1);"">" & IDValue & "</a></center></nobr></TD>")
