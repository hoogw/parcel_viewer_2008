...
'Important''''''''''''''''''''''''''''''''
	...
	session("Fld_DB")="APN"
	session("Fld_SHP")="APN"
	session("Fld_DBNAMES")="APN,c_owners,c_address"
	session("ds")=Application("dbSource") '"Provider=SQLOLEDB.1;Initial Catalog=tuetin2;Data Source=GEOPRISE1;User Id=tustin;Password=Tustin123;"
	session("DBTbl")="assess5"
	''''''''''''''''''''''''''''''''''''''''''
...
<%
    ...
    strSQL = "Select o.APNbr,m.full_address,[owner] from " & Trim(session("DBTbl")) & " where o.apnbr = m.APN and "
    ...
    If Trim(Request.Form("QueryString")) = "" Then
		...
		strSQL &= " ORDER BY m.full_address"
    Else
        strSQL = Trim(Request.Form("QueryString"))
    End If

	session("o")=trim(request.form("SAfld0"))
