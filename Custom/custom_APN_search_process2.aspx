...
<% 
	...
	session("Fld_DB")="AIN"
	session("Fld_SHP")="AIN"
	session("Fld_DBNAMES")="AIN,sa_house_n,sa_street_,zst_owner1"
...
<body leftmargin="5" onload="top.HaltLYRHide=0; top.MapFrame.hideLoadingLyr();">
		...
		<script language="javascript">
			...
			function SendValue(fnd,APN,QL) {
				...
				var url = "custom/custom_parcel_details.aspx?APN=" + APN
				top.MapFrame.url = "";
...
strSQL = "Select " & trim(session("Fld_DBNAMES")) & " from " & Trim(session("DBTbl")) & " where "
	...
	if trim(session("APNList"))<> "" then
		...
		strSQL ="SELECT " & trim(session("Fld_DBNAMES")) & " FROM " & Trim(session("DBTbl")) & " WHERE AIN in (" & APNList & ") ORDER BY AIN"
	response.write ("<!--- SQL: " & strSQL & " --->")
...
'		strSQL = "Select " & trim(session("Fld_DBNAMES")) & " from " & Trim(session("DBTbl")) & " where " & strSQL & " ORDER BY AIN"
	response.write ("<!--- SQL: " & strSQL & " --->")
		strSQL = "SELECT " &trim(session("Fld_DBNAMES")) & " FROM " & Trim(session("DBTbl")) & " WHERE " & strSQL & " ORDER BY AIN"
	end if
    Else
    ...
    If Err.Number = 0 Then
        ...
        Write(intFileNum, "Addr #")
        Write(intFileNum, "Street")
		...
		response.write ("<TH class=tblHdr>Addr #</TH>")
