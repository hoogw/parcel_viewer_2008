...
<% 
	...
	session("ds")=Application("dbSource") '"Provider=SQLOLEDB.1;Initial Catalog=Costa2;Data Source=srv6;User Id=CostaMesa;Password=CostaMesa;"
...
<table width=98% cellpadding=0 cellspacing=0 border=0 ID="Table3"><tr class=tblHdr>
			<td align=center><b>APN Search Results</b></font></td></tr></table>
<br>
...
strSQL = "Select " & trim(session("Fld_DBNAMES")) & " from " & Trim(session("DBTbl")) & " where "
	...
	if trim(session("APNList"))<> "" then
		...
		strSQL ="SELECT " & trim(session("Fld_DBNAMES")) & " FROM " & Trim(session("DBTbl")) & " WHERE APNlink2 in (" & APNList & ") ORDER BY APNlink2"
	response.write ("<!--- SQL: " & strSQL & " --->")

	else
		if trim(request.form("SAfld0")) <> "" then
			strSQL = Trim(CStr(Request.Form("SAfld0Name"))) & " like '" & Trim(CStr(replace(Request.Form("SAfld0"),"-",""))) + "%' "
	response.write ("<!--- SQL: " & strSQL & " --->")

		end if
		if trim(request.form("SAfld1")) <> "" then
			strSQL = Trim(CStr(Request.Form("SAfld0Name"))) & " >= '" & Trim(CStr(replace(Request.Form("SAfld0"),"-",""))) + "' AND "
			strSQL &= Trim(CStr(Request.Form("SAfld1Name"))) & " <= '" & Trim(CStr(Request.Form("SAfld1"))) + "' "
	response.write ("<!--- SQL: " & strSQL & " --->")
		end if

'		strSQL = "Select " & trim(session("Fld_DBNAMES")) & " from " & Trim(session("DBTbl")) & " where " & strSQL & " ORDER BY APN"
	response.write ("<!--- SQL: " & strSQL & " --->")
		strSQL = "SELECT " &trim(session("Fld_DBNAMES")) & " FROM " & Trim(session("DBTbl")) & " WHERE " & strSQL & " ORDER BY APN"
	end if
    Else
...
'	response.write (strSQL)
 ...
 response.write ("</TABLE><br>")
		...
		response.write ("<TH class=tblHdr>Parcel ID</TH></TD>")
		response.write ("<TH class=tblHdr>Owner</TH></TD>")
		response.write ("<TH class=tblHdr>Address</TH></TD>")
        response.write ("</TR>")
		...
		For i = nStart To nEnd
            ...
            For j = 0 To ubound(FieldNamestoDisplay)
				if isDBNull(DT.Rows(i)(j))=false then
					...
					select case j
					case 0
						response.write ("<TD width='40%'><center><a href=javascript:SendValue(""" & Trim(CStr(session("Fld_SHP"))) & """,""" & thisvalue & """,""" & CustLayer & """);>" & thisvalue & "</a></center></TD>")
						FirstRec = "SendValue(""" & Trim(CStr(session("Fld_SHP"))) & """,""" & thisvalue & """,""" & CustLayer & """)"
					case else
						response.write ("<TD width='60%'>" & thisvalue & "</TD>")
					end select
