...
<% 
	...
	'Important''''''''''''''''''''''''''''''''
	session("lyr")="2"
	session("Fld_DB")="APN,Address,Owner"
	session("Fld_SHP")="APN"
	session("Fld_DBNAMES")="APN,Site_Addre,Owner"
	session("ds")="Provider=SQLOLEDB.1; Initial Catalog=Murietta;Data Source=actgis6;User Id=Murietta;Password=Murietta"
...
<%
    ...
    strSQL = "Select APN,Site_Addre,Owner from " & Trim(session("DBTbl")) & " where "
    ...
    If Trim(Request.Form("QueryString")) = "" Then
		...
		strSQL = "Select APN,Site_Addre,Owner from " & Trim(session("DBTbl")) & " where " & strSQL & " ORDER BY APN"
